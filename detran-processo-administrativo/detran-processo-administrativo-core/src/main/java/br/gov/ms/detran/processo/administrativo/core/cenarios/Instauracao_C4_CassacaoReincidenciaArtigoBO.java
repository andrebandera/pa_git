 ...
 * @author Lillydi
 */
public class Instauracao_C4_CassacaoReincidenciaArtigoBO extends InstauracaoCenario implements IInstauracaoCenario {

    private static final Logger LOG = Logger.getLogger(Instauracao_C4_CassacaoReincidenciaArtigoBO.class);

    private final RegrasGeralInstauracaoCondutorBO regraGeralCondutorBO;

    public Instauracao_C4_CassacaoReincidenciaArtigoBO() {
        super();
    ...
    public IBaseEntity executarInstauracao(EntityManager em, InstaurarProcessoAdministrativoWrapper wrapper) throws AppException {
        ...
        if (!DetranCollectionUtil.ehNuloOuVazio(wrapper.getInfracoes())) {
            ...
            DadosInfracaoPAD retorno = buscarInfracao(em, wrapper, pasSuspensao);

            if (retorno != null) {
                infracoes.add(retorno);
            }
        }
        ...
        if (!DetranCollectionUtil.ehNuloOuVazio(listaInfracoes)) {
            ...
            for (DadosInfracaoPAD infracao : listaInfracoes) {
                ...
                try {
                    ...
                    wrapper
                        .setProcesssoAdministrativoOrigem(identificaProcessoAdministrativoOrigem(em, infracao, pasSuspensao)
                        );
    ...
    private ProcessoAdministrativo identificaProcessoAdministrativoOrigem(
        ...
        ProcessoAdministrativo paOrigem = 
            regraGeralInfracaoBO
                .validarMesmoArtigoInfracaoPeriodo1Ano(em, infracao, pasSuspensao);
