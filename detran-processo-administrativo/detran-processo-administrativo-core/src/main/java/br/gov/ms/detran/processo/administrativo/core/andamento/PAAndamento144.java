...
public class PAAndamento144  extends ExecucaoAndamento {
    
    private static final Logger LOG = Logger.getLogger(PAAndamento144.class);
    

    @Override
    public RetornoExecucaoAndamentoWrapper executaEspecifico(EntityManager em, ExecucaoAndamentoEspecificoWrapper wrapper) throws AppException {
        
        LOG.debug("Início Andamento 144.");
        
        RetornoExecucaoAndamentoWrapper retornoWrapper = 
                new DesistenciaRecursoInstauracaoPenalizacaoBO()
                        .definirDesistenteRecursoInstPen(em, wrapper.getProcessoAdministrativo());
        
        if (null == retornoWrapper) {
            if(RegraInstaurarEnum.C1.equals(wrapper.getProcessoAdministrativo().getOrigemApoio().getRegra())){
                retornoWrapper = 
                    new RetornoExecucaoAndamentoWrapper(TipoRetornoAndamentoEnum.MUDANCA_FLUXO_ANDAMENTO, 
    ...
    public void validaEspecifico(EntityManager em,ExecucaoAndamentoEspecificoWrapper wrapper) throws AppException {
        ...
        if(!TipoFasePaEnum.INSTAURACAO.equals(movimento.getRecurso().getTipoRecurso())){
